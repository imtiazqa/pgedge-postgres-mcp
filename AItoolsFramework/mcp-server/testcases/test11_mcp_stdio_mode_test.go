...
// ============================================================================
// MCP Server Stdio Mode Tests
// ============================================================================

func (s *MCPServerTestSuite) testStdio_ModeWithDatabaseConnectivity() {
	s.T().Log("Testing MCP server in stdio mode with database connectivity...")
	...
	// ====================================================================
	s.T().Log("Step 0: Stopping any running MCP server instances...")
	...
	if canUseSystemd {
		output, _, _ := s.ExecCommand("systemctl is-active pgedge-postgres-mcp.service 2>/dev/null")
		if strings.Contains(output, "active") {
			s.T().Log("  Stopping systemd service...")
			s.ExecCommand("systemctl stop pgedge-postgres-mcp.service")
			time.Sleep(2 * time.Second)
		}
	}

	// Kill any manually started processes
	s.ExecCommand("pkill -f pgedge-postgres-mcp || true")
	...
	// ====================================================================
	s.T().Log("Step 1: Creating stdio mode configuration...")

	// Get database config from suite config
	...
	stdioConfig := fmt.Sprintf(`cat > /tmp/postgres-mcp-stdio-test.yaml << 'EOF'
databases:
  - host: %s
    port: %d
    name: %s
...
server:
  mode: stdio
EOF`, dbHost, dbPort, dbName, dbUser, dbPassword)

	output, exitCode, err := s.ExecCommand(stdioConfig)
	...
	output, exitCode, err = s.ExecCommand("grep 'mode: stdio' /tmp/postgres-mcp-stdio-test.yaml")
	...
	// ====================================================================
	s.T().Log("Step 2: Creating stdio test script...")
	...
	testScript := fmt.Sprintf(`cat > /tmp/test-stdio.sh << 'SCRIPT'
#!/bin/bash

# Timeout after 5 seconds total
timeout 5 %s -config /tmp/postgres-mcp-stdio-test.yaml << 'INPUT' > /tmp/stdio-output.log 2>&1
...
SCRIPT`, mcpBinary)
	...
	output, exitCode, err = s.ExecCommand(testScript)
	...
	// ====================================================================
	s.T().Log("Step 3: Starting MCP server in stdio mode...")

	output, exitCode, err = s.ExecCommand("bash /tmp/test-stdio.sh")

	// Read the output log to verify responses
	logOutput, logExitCode, logErr := s.ExecCommand("cat /tmp/stdio-output.log")
	...
	// ====================================================================
	s.T().Log("Step 4: Verifying MCP server database connectivity in stdio mode...")
	...
	// ====================================================================
	s.T().Log("Step 5: Executing database query to confirm MCP server connected to DB...")

	// Create a more comprehensive test that actually queries the database
	queryTestScript := fmt.Sprintf(`cat > /tmp/test-stdio-query.sh << 'SCRIPT'
#!/bin/bash

# Timeout after 6 seconds total
timeout 6 %s -config /tmp/postgres-mcp-stdio-test.yaml << 'INPUT' > /tmp/stdio-query-output.log 2>&1
{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test-client","version":"1.0.0"}}}
...
SCRIPT`, mcpBinary)
	...
	output, exitCode, err = s.ExecCommand(queryTestScript)
	...
	// Run the query test
	output, exitCode, err = s.ExecCommand("bash /tmp/test-stdio-query.sh")

	queryLogOutput, logExitCode, logErr := s.ExecCommand("cat /tmp/stdio-query-output.log")
	if logErr == nil && logExitCode == 0 {
	...
	// ====================================================================
	s.T().Log("Step 6: Stopping MCP server after testing...")
	...
	s.ExecCommand("pkill -9 -f 'timeout.*stdio-test.yaml' || true")
	s.ExecCommand(fmt.Sprintf("pkill -9 -f '%s.*stdio-test.yaml' || true", mcpBinary))
	time.Sleep(2 * time.Second)

	// Verify all processes are gone using specific pattern that won't match DNF
	for attempt := 1; attempt <= 5; attempt++ {
		output, exitCode, err = s.ExecCommand(fmt.Sprintf("pgrep -f '%s.*stdio-test.yaml' || echo 'no-process'", mcpBinary))
		...
		if pids != "" && pids != "no-process" {
			...
			if len(pidList) > 0 {
				...
				for _, pid := range pidList {
					psOut, _, _ := s.ExecCommand("ps -fp " + pid + " 2>/dev/null || echo 'process-gone'")
			...
			s.ExecCommand("sudo kill -9 " + allPids + " 2>/dev/null || kill -9 " + allPids + " 2>/dev/null || true")
		}
		time.Sleep(2 * time.Second)
	}
	...
	if !strings.Contains(output, "no-process") {
		...
		if len(pidList) > 0 {
			...
			s.ExecCommand("sudo kill -9 " + allPids + " 2>/dev/null || true")
			s.ExecCommand("kill -9 " + allPids + " 2>/dev/null || true")
			time.Sleep(1 * time.Second)

			// Check one more time
			output, _, _ = s.ExecCommand(fmt.Sprintf("pgrep -f '%s.*stdio-test.yaml' || echo 'no-process'", mcpBinary))
			if !strings.Contains(output, "no-process") {
	...
	// ====================================================================
	s.T().Log("Step 7: Cleaning up test files...")

	s.ExecCommand("rm -f /tmp/postgres-mcp-stdio-test.yaml")
	s.ExecCommand("rm -f /tmp/test-stdio.sh")
	s.ExecCommand("rm -f /tmp/test-stdio-query.sh")
	s.ExecCommand("rm -f /tmp/stdio-output.log")
	s.ExecCommand("rm -f /tmp/stdio-query-output.log")
	...
	s.T().Logf("  • Database connection: MCP server connected to PostgreSQL at %s:%d", dbHost, dbPort)
